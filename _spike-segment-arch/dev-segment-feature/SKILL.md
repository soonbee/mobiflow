---
name: dev-segment-feature
description: 범용 기능 개발·수정 세그먼트 플레이북. dev-segment-router가 feature를 반환했을 때 이 스킬을 로드하여 구현-검증-리뷰 플로우를 실행한다.
user-invocable: false
---

# dev-segment-feature

UI 시안 없이 범용 기능을 구현·수정하는 세그먼트 플레이북.

본 playbook은 `run-dev-from-ticket`이 `Skill` tool로 load해 사용한다. **오케스트레이터(부모 스킬)가** 1-1 → 1-2 → 2-1 → 2-2 → 3-1을 **순서대로**, **각 단계마다 별도의 `Task` 호출**로 실행한다. 단계 통합·생략 금지(특히 1-1 `eng-feature`에 formatter·static-fixer·reviewer 책임을 떠넘기지 마라). 루프 A는 3-1 종료 후 오케스트레이터가 판정한다.
...
서브에이전트 `eng-feature`를 호출하여 기능을 구현한다.

- **입력**: 티켓 내용
- **수행 내용**:
  - API 계약(스키마/타입)을 먼저 정의하거나 기존 계약을 확인한다
  - 계약 기반으로 기능을 구현한다
  - 기존 코드 수정 시 영향 범위를 파악하고 변경한다
- **출력**: 구현/수정된 코드 파일
...
- **계약 미충족 (contract-missing)**: Expo 프로젝트에서 스크립트가 없으면 루프 A 편입. 비-Expo 프로젝트는 향후 스택별 스킬이 도입되기 전까지 관리자가 단계 적용 여부를 결정
...
서브에이전트 `reviewer-code`를 호출하여 **코드 품질(Part A) + 티켓 AC(Part B)** 를 한 번에 점검한다.

- **수행 내용**:
  - Part A — 네이밍, 타입, 에러 처리, 가독성, 중복, 성능 등 코드 품질 점검
...
- **조건**: 다음 중 하나라도 해당:
  ...
  - reviewer-code(3-1) Part A 또는 Part B 🔴 수정 필요 항목 존재
- **복귀**: → 1-1. eng-feature
- **컨텍스트**: 각 단계의 실패 메시지(정적 오류 목록·metro-load 로그·리뷰 Part A·B 피드백)를 모두 수집해 eng-feature에 전달
