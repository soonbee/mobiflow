...
description: >
  ...
  사용자가 `/mobiflow:ticket-edit {N}`을 입력할 때 트리거하세요.
...
> 먼저 `/mobiflow:ticket`으로 티켓을 생성해주세요.
...
| 형식 | 동작 |
|---|---|
| `/mobiflow:ticket-edit {N}` | 자연어 요청을 추가로 묻고 분류 게이트 진입 |
| `/mobiflow:ticket-edit {N} {자유 설명}` | 즉시 분류 게이트 진입 |

번호는 **현재 PRD 버전 기준** 패딩 없는 정수. 다른 버전을 편집하려면 PRD 버전을 먼저 정렬해야 한다.

복수 티켓(`/mobiflow:ticket-edit 7,11`)은 미지원 — 한 번에 한 티켓만 처리. 여러 티켓 편집이 필요하면 본 스킬을 반복 호출.
...
| `ready` | 거부. "worktree에서 수정하거나 `/mobiflow:merge-ticket {N}` 후 후속 티켓으로 처리하세요" |
| `done` | 거부. "후속 티켓 권유: `/mobiflow:ticket "{설명}"` (kind: bugfix/refactor, 의존: {N})" |

**`failed` 진입 시 추가 안내**:

> ⚠️ 티켓 #{N} 상태가 `failed`입니다. 본문·메타 편집은 허용되지만 상태 전이는 본 스킬에서 처리하지 않습니다.
> 편집 완료 후 재실행: worktree(`worktrees/t{N}`)·feature 브랜치를 사용자가 직접 제거한 뒤 `_index.md` 상태를 `pending`으로 되돌리고 `/mobiflow:dev-from-ticket {N}` 호출.
...
>   - #{X1}, #{X2}, ...
>
> 이 티켓들의 의존 그래프는 자동 재배선되지 않습니다. 필요 시 `/mobiflow:ticket-edit {X1}`로 의존을 정리하세요.
...
```
다음 단계:
  - 구현 진입:   /mobiflow:dev-from-ticket {N}
  - 추가 수정:   /mobiflow:ticket-edit {N}
```
...
```
다음 단계:
  - 의존 정리:   /mobiflow:ticket-edit {X}   # #{N}을 의존하던 티켓이 있는 경우
  - 복원 필요:   /mobiflow:ticket-edit {N}
```
...
```
다음 단계:
  - 구현 진입 (신규):   /mobiflow:dev-from-ticket {NEW_NUMBERS}
  - 추가 수정 (신규):   /mobiflow:ticket-edit {NEW_N_one}
...
```
다음 단계:
  - 구현 진입:   /mobiflow:dev-from-ticket {NEW_N}
  - 추가 수정:   /mobiflow:ticket-edit {NEW_N}
...
> - `/mobiflow:ticket-edit` 추가 호출은 working tree를 더 키웁니다
