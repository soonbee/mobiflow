---
name: use-unistyles3
description: Unistyles 3 기반 React Native 스타일링. StyleSheet.create를 react-native-unistyles에서 import하고, rt.insets/rt.screen/theme 등 Unistyles 3 API를 활용한다. dev-from-ticket이 stack-resolver 매칭 시 expo-sdk55-unistyles3 프로파일을 통해 로드.
---
...
## configure 시점 주의 (metro-load smoke 대응)

`StyleSheet.create()`는 Unistyles `configure()`가 먼저 실행된 후에만 안전하게 호출 가능. configure가 완료되기 전 모듈 평가 시점에 `StyleSheet.create()`가 호출되면 metro-load smoke에서 module-evaluation-order 크래시 발생.

대응:

- `configure()`는 앱 진입점(예: `app/_layout.tsx` 최상단 또는 `index.ts`)에서 한 번만 호출
- 컴포넌트 파일의 top-level `StyleSheet.create()`는 configure 이후에만 평가되도록 import 순서 보장
- 의심 시 `npm run smoke:min`(또는 `metro-load:cold-require`)으로 검증

`expo-sdk55-unistyles-stack` scaffolder가 심은 smoke 스크립트가 이를 자동 검증.
