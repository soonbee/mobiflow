...
디스크 스캔 (package.json, lockfile)   ┘                            └→ Makefile (nidost:scope-includes 블록만)
기존 project.config.yaml (보존 필드)                                (단일 commit)
```

산출물은 두 개로 한정: (1) `docs/project.config.yaml` 전체, (2) 루트 `Makefile`의 `# >>> nidost:scope-includes >>>` 블록. 두 출력 모두 같은 입력(`repo.scopes.*.path`)에서 파생되므로 함께 갱신·commit 합니다.
...
- **단일 책임**: 본 스킬은 docs를 **읽기만** 한다. 절대 수정하지 않음. 컴파일 산출물은 두 개로 한정: `docs/project.config.yaml`과 루트 `Makefile`의 `# >>> nidost:scope-includes >>>` 블록. 그 외 파일은 staging하지 않음.
...
> ❌ `docs/architecture/architecture.md`가 없습니다. compile-project-config는 architecture가 작성된 후에 의미가 있습니다 (repo 토폴로지·스택 결정의 SSOT). 먼저 `/nidost:spec-architecture`를 실행하세요.
...
> ❌ `docs/project.config.yaml`이 없습니다. `/nidost:init`이 생성해야 하는 파일입니다. init을 다시 실행하거나 수동으로 템플릿을 복사하세요.

### 0-5. 루트 Makefile + marker 블록 존재 확인

```bash
test -f Makefile \
  && grep -q "# >>> nidost:scope-includes >>>" Makefile \
  && grep -q "# <<< nidost:scope-includes <<<" Makefile
```

실패하면 종료:

> ❌ 루트 `Makefile`이 없거나 `nidost:scope-includes` marker 블록이 누락됐습니다. `init`이 생성한 형태로 복원하세요(`skills/init/templates/Makefile` 참고). 본 스킬은 marker 블록 안만 재생성하므로 두 marker 라인이 모두 필요합니다.
...
- 그 외 인자 → 에러 후 종료:
  ...
  > ❌ 알 수 없는 인자입니다. 사용법: `/nidost:compile-project-config` 또는 `/nidost:compile-project-config --check`
...
`Makefile`을 읽고 `# >>> nidost:scope-includes >>>` 다음 줄부터 `# <<< nidost:scope-includes <<<` 직전 줄까지의 내용을 메모리에 보관합니다. 빈 블록(라인 0개)도 정상. STEP 4 diff 비교에 사용.
...
❌ drift 감지
  ...
  Makefile (nidost:scope-includes):
    - -include apps/mobile/Makefile.targets
...
갱신: /nidost:compile-project-config (인자 없이)
```
