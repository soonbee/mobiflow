...
description: PRD의 페르소나를 기반으로 사용 시나리오 문서(user-journey.md)를 생성하는 스킬. 사용자가 "/mobiflow:user-journey", "사용 시나리오 작성", "페르소나 시나리오", "유저 저니"를 언급할 때 반드시 트리거하세요.
...
mobiflow의 기획·설계 체인은 프로젝트 유형과 무관하게 고정 순서로 진행합니다. PRD에서 이 단계의 canonical form(페르소나별 사용 시나리오)이 프로젝트에 직접 적용되지 않는다고 판단되면, 단계를 **건너뛰지 말고** 재해석해 문서를 작성합니다.
...
재해석할 의미 있는 대체 관심사가 전혀 없는 케이스(순수 라이브러리 SDK 등)에서는 `doc-guide.md` 「재해석 모드의 두 형태」의 **A2형 (스킵 선언)**을 따른다. §0 범위 선언에 작성 스킵 사유를 한 단락으로 명시하고, 본문은 "이 프로젝트에서는 사용자 시나리오 개념이 해당 없음"으로 축약한다.
...
> ℹ️ `prd`가 v{version} Working 상태입니다.
>    이 문서는 prd@{version}(Working) 기반으로 작성되며,
>    모든 spec 작성을 마친 뒤 `/mobiflow:spec-lock`으로 일괄 Lock하세요.
>
> 1. 그대로 진행 (Working 참조)
> 2. 지금 Lock (`/mobiflow:spec-lock prd` 실행)
> 3. 종료

- **1번**: 그대로 진행하되 `based_on`에 해당 버전을 기록하고 `(Working)` 마커를 함께 표기
- **2번**: spec-lock 스킬을 호출해 선행 문서를 Lock → 완료 후 STEP 0-1로 계속
- **3번**: 종료

여러 선행 문서가 Working 상태이면 하나씩 순차로 물어봅니다.
...
# 있으면 version 추출
VERSION=$(awk '/^version:/ {print $2; exit}' docs/user-journey/user-journey.md)
git tag --list "doc/user-journey/v${VERSION}" | grep -q . && echo "locked" || echo "working"
```
...
> ℹ️ `docs/user-journey/user-journey.md`가 v{version} Working 상태입니다.
>
...
> ⚠️ `docs/user-journey/user-journey.md` (v{기존버전})가 Lock 상태입니다.
>
...
전체 초기화가 필요하면 사용자가 수동으로 `rm -rf docs/user-journey/` 후 스킬을 재호출합니다.
...
```
✅ mobiflow user-journey v0.1.0 작성 완료
...
다음 스킬 후보: mobiflow:spec-architecture
```
