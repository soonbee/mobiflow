...
description: >
  ...
  `/nidost:refine` 슬래시 커맨드로 명시 호출할 때만 실행되며 자연어 언급으로 자동
  트리거되지 않는다.
...
| 형식 | 동작 |
|---|---|
| `/nidost:refine "<자연어 요청>"` | 인자로 받은 요청을 즉시 처리 |
| `/nidost:refine` | STEP 1에서 자유 입력 받음 |
...
>
> 1. 지금 Lock (`/nidost:spec-lock {카테고리}`)
> 2. 추가 수정 후 한 번에 Lock
> 3. 종료 (Working 유지)
...
다음 단계:
  - 추가 손질: /nidost:refine
  - spec Lock (필요 시): /nidost:spec-lock <카테고리>
```
...
다음 단계:
  - 다음 spec 라운드 검토: /nidost:spec-prd 시작 시 백로그 사용
  - 범위를 좁혀 재요청: /nidost:refine "<좁힌 요청>"
```
