...
- **캡처 실패 ≠ 빌드 실패**: HTTP 서버 경유 필수(`data-include` 탓). Playwright/캡처가 실패해도 빌드·리뷰 산출물은 유효. PNG의 주 소비자는 후속 implement phase의 design-diff 증적이며 draft 내부 리뷰는 `INDEX.html` 라이브 iframe이 primary view.
...
각 (screen, variant, viewport)마다 **뷰포트 샷**을 기본 1장, 스크롤 비율(`scrollHeight / viewport.height`)이 1.1 초과면 **풀 샷**을 추가 1장. 비율이 5를 넘으면 풀 샷은 뷰포트 높이 × 5로 컷. 뷰포트 샷이 design-diff primary, 풀 샷이 secondary(접지 콘텐츠 완결성 보조).
...
- PNG 캡처는 INDEX에서 빠지고 **후속 implement phase의 design-diff 증적**이 주 목적. INDEX는 PNG 링크를 노출하지 않음
