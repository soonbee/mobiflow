...
description: PRD·architecture·db-design을 분석해 API 명세를 설계하는 스킬. 사용자가 "/mobiflow:spec-api-design", "API 설계", "API 명세", "엔드포인트 설계", "REST API 설계"를 언급할 때 반드시 트리거하세요.
...
mobiflow의 기획·설계 체인은 프로젝트 유형과 무관하게 고정 순서로 진행합니다. PRD·architecture에서 이 단계의 canonical form(HTTP/REST API 명세)이 프로젝트에 직접 적용되지 않는다고 판단되면, 단계를 **건너뛰지 말고** 재해석해 문서를 작성합니다.
...
재해석할 의미 있는 대체 인터페이스가 없는 케이스에서는 `doc-guide.md` 「재해석 모드의 두 형태」의 **A2형 (스킵 선언)**을 따른다. §0 범위 선언에 작성 스킵 사유를 한 단락으로 명시하고, §1~§6은 "이 프로젝트에서는 외부 인터페이스가 해당 없음"으로 축약한다.
...
이 스킬의 필수 선행 문서는 `prd`, `architecture`, `db-design`입니다. 각 선행 문서에 대해 파일 존재 + 태그 존재 여부를 확인해 Working 상태인 것을 모두 찾아냅니다.

```bash
for CAT in prd architecture db-design; do
...
> ❌ `docs/architecture/architecture.md`를 찾을 수 없습니다. 인증 방식·Base URL·인프라 제약을 확정한 뒤에 API를 설계해야 합니다. 먼저 `/mobiflow:spec-architecture`를 실행해주세요.
...
> ❌ `docs/db-design/db-design.md`를 찾을 수 없습니다. 테이블 구조와 PK 전략이 확정되어야 엔드포인트 도메인과 ID 표현을 결정할 수 있습니다. 먼저 `/mobiflow:spec-db-design`을 실행해주세요.
...
```bash
test -f docs/api-design/api-design.md
VERSION=$(awk '/^version:/ {print $2; exit}' docs/api-design/api-design.md)
git tag --list "doc/api-design/v${VERSION}" | grep -q . && echo "locked" || echo "working"
```
...
> ℹ️ `docs/api-design/api-design.md`가 v{version} Working 상태입니다.
>
...
> ⚠️ `docs/api-design/api-design.md` (v{기존버전})가 Lock 상태입니다.
>
...
전체 초기화가 필요하면 사용자가 수동으로 `rm -rf docs/api-design/` 후 스킬을 재호출합니다.
...
```
✅ mobiflow api-design v0.1.0 작성 완료
...
다음 단계: /mobiflow:run-draft-from-spec (draft phase)
