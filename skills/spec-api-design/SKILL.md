...
- **인터뷰 총량 상한: 권장 4개, 강제 6개**. 6개 도달 시 남은 축은 자동 도출 기본값으로 결정한다
- 한 번에 질문 하나만 던지고, 답변이 충분히 구체적이면 다음 단계로 넘어간다
- **비개발자 친화 톤**: 기술 용어가 등장하면 한 줄로 풀어 설명한다 (예: "페이지네이션 = 데이터 목록을 한 번에 다 주지 않고 여러 페이지로 나눠 제공하는 방식")

### 앵커 패턴 (매 응답 필수 준수)

모든 응답은 반드시 아래 구조를 따릅니다:

1. **컨텍스트 요약** — 이 질문이 왜 필요한지, 결정이 무엇에 영향을 주는지 한 줄
2. **핵심 질문 1개** — 비개발자도 이해할 수 있는 표현
3. **선택지 2~4개**
   - `(추천)` 태그 + 1~2문장 이유
   - 각 선택지에 "이렇게 되면 실제로 어떻게 동작하는지" 한 줄 설명
   - 항상 '직접 입력' 포함
4. **대기** — 응답 생성 중단

### 결정 축 (질문 우선순위)

1. **버저닝 전략** — 나중에 API가 바뀔 때 어떻게 호환성을 유지할지 (URL 경로 `/v1` / 헤더 / 무버저닝)
2. **응답 포맷(envelope)** — 모든 응답을 `{success, data, error}`로 감쌀지, 데이터를 그대로 줄지
3. **페이지네이션** — 데이터 목록이 많을 때 어떻게 쪼갤지 (offset 기반·cursor 기반·미사용)
4. **에러 코드 체계** — HTTP 상태 코드만 쓸지, 앱 고유 코드(`ERR_USER_NOT_FOUND`)도 함께 줄지
5. **파일 업로드 방식** — 서버에 직접 올릴지, 클라우드 스토리지에 직접 올리는 presigned URL 방식을 쓸지

위 5개 축 중 STEP 1-4에서 자동 도출된 것은 모두 스킵합니다.
...
- 모든 축을 추천 기본값으로 자동 결정합니다
- 기술 결정 로그의 `근거` 컬럼에 `(자동 선택)` 표기를 추가합니다

### 직접 입력 처리

사용자가 선택지 외 의견을 입력할 경우:

- **비판적 검토**: 실현 가능성과 PRD/architecture/db-design과의 일관성 평가. 맹목적 수용 금지
- **모호함 해결**: 입력이 모호하면 다음 단계로 넘어가기 전 후속 질문 1개로 명확화
- **피벗 브리핑**: 결정이 바뀌면 영향 범위(엔드포인트 재설계·에러 코드·클라이언트 코드)를 요약 후 진행

### 기술 결정 로그

인터뷰 중 결정된 항목과 STEP 1-4에서 자동 도출된 항목 모두 내부적으로 기록해 두었다가 STEP 3에서 `## 6. 기술 결정 로그` 섹션에 반영합니다.
...
| 결정 축     | 결정                                | 근거                                                 | 제쳐진 후보          |
| ----------- | ----------------------------------- | ---------------------------------------------------- | -------------------- |
| 버저닝 전략 | URL 경로(`/v1`)                     | 가장 명시적, 클라이언트 디버깅 용이 (자동 선택)      | 헤더 방식, 무버저닝  |
| 응답 포맷   | `{success, data, error}` envelope   | 프론트엔드 에러 핸들링 일관화                        | bare, RFC 9457       |
| ...         | ...                                 | ...                                                  | ...                  |

자동 도출된 결정은 `근거` 컬럼에 `(자동 선택)` 표기를 추가합니다. 인터뷰로 결정된 항목은 사용자 답변을 그대로 또는 요약해 기록합니다.
