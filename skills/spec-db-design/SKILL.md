...
- **비개발자 친화 톤**: 기술 용어가 등장하면 한 줄로 풀어 설명한다 (예: "soft delete = 데이터를 실제로 지우지 않고 '삭제됨' 표시만 남기는 방식")
...
위 5개 축 중 STEP 1-3에서 자동 도출된 것은 모두 스킵합니다.
...
- **비판적 검토**: 실현 가능성과 PRD/architecture와의 일관성 평가. 맹목적 수용 금지
- **모호함 해결**: 입력이 모호하면 다음 단계로 넘어가기 전 후속 질문 1개로 명확화
- **피벗 브리핑**: 결정이 바뀌면 영향 범위(테이블 추가·인덱스·마이그레이션)를 요약 후 진행

### 기술 결정 로그

인터뷰 중 결정된 항목과 STEP 1-3에서 자동 도출된 항목 모두 내부적으로 기록해 두었다가 STEP 3에서 `## 7. 기술 결정 로그` 섹션에 반영합니다.
...
| 결정 축 | 결정 | 근거 | 제쳐진 후보 |
| ------- | ---- | ---- | ----------- |
| ID 전략 | BIGINT AUTO_INCREMENT | 단일 인스턴스 운영, ID 외부 노출 없음 (자동 선택) | UUID, ULID |
| ...     | ...  | ...  | ...         |
