...
> ⚠️ `docs/db-design/db-design.md`가 이미 존재합니다.
>
...
- **1번 선택**: 기존 `docs/db-design/db-design.md`와 `docs/db-design/CHANGELOG.md`를 삭제하고 STEP 1로 진행
- **2번 선택**(또는 그 외 응답): 종료
- 1번 선택 후 삭제는 다음 명령으로 수행:

  ```bash
  rm -f docs/db-design/db-design.md docs/db-design/CHANGELOG.md
  ```
...
1. **데이터 규모와 성장 속도** — 1년 후 핵심 테이블의 row 수 예상치 (인덱스/파티셔닝 결정 입력)
2. **삭제 정책** — 사용자가 데이터를 지울 때 완전 삭제 vs 휴지통(복구 가능)
3. **이력·감사 추적** — 누가 언제 무엇을 바꿨는지 기록 필요 여부 (규제·금융이면 강하게 권장)
4. **멀티테넌시** — 여러 회사·조직이 한 시스템을 사용하는지, 데이터를 어떻게 격리할지
5. **ID 노출** — URL이나 API 응답에 ID가 그대로 노출되는지 (노출되면 UUID/ULID 권장)
...
- 이 스킬은 **신규 db-design 최초 작성 전용**입니다. 기존 파일이 있으면 STEP 0에서 사용자에게 "삭제 후 재작성" 여부만 묻고, 부분 수정은 지원하지 않습니다.
