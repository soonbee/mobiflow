---
name: spec-db-design
description: PRD와 architecture를 분석해 DB 스키마와 ERD를 설계하는 스킬. 사용자가 "/mobiflow:spec-db-design", "DB 설계", "데이터베이스 스키마", "ERD 설계", "스키마 설계"를 언급할 때 반드시 트리거하세요.
...
mobiflow의 기획·설계 체인은 프로젝트 유형과 무관하게 고정 순서로 진행합니다. PRD·architecture에서 이 단계의 canonical form(관계형 DB 스키마)이 프로젝트에 직접 적용되지 않는다고 판단되면, 단계를 **건너뛰지 말고** 재해석해 문서를 작성합니다.
...
> ❌ `docs/architecture/architecture.md`를 찾을 수 없습니다. DB 기술과 인프라 제약을 확정한 뒤에 스키마를 설계해야 합니다. 먼저 `/mobiflow:spec-architecture`를 실행해주세요.
...
```
✅ mobiflow db-design v0.1.0 작성 완료

  문서:              docs/db-design/db-design.md
  기준 PRD:          docs/prd/prd.md (v{PRD_VERSION})
  기준 Architecture: docs/architecture/architecture.md (v{ARCH_VERSION})
  CHANGELOG:         docs/db-design/CHANGELOG.md 갱신
  INDEX:             docs/INDEX.md 갱신

다음 스킬 후보: mobiflow:spec-api-design
```
