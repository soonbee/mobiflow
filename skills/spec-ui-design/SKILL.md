---
name: spec-ui-design
description: PRD·user-journey·architecture·design-tokens를 종합해 화면 목록·사이트맵·화면별 명세·컴포넌트 매핑을 설계하는 스킬. 사용자가 "/nidost:spec-ui-design", "UI 설계", "화면 설계", "사이트맵", "와이어프레임 명세"를 언급할 때 반드시 트리거하세요.
...
> ❌ `docs/design-tokens/design-tokens.md`를 찾을 수 없습니다. 화면별 명세의 UI 요소·공통 컴포넌트 매핑은 design-tokens의 variant 카탈로그를 참조해야 합니다. 먼저 `/nidost:spec-design-tokens`를 실행해주세요.
...
> ⚠️ `docs/ui-design/ui-design.md` (v{기존버전})가 이미 존재합니다.
>
...
- **2번 선택**: 기존 `docs/ui-design/ui-design.md`와 `docs/ui-design/CHANGELOG.md`를 삭제하고 STEP 1로 진행
- **3번 선택**(또는 그 외 응답): 종료
