...
description: PRD를 분석해 시스템 아키텍처와 기술 스택을 설계하는 스킬. 사용자가 "/mobiflow:spec-architecture", "아키텍처 설계", "시스템 디자인", "기술 스택 선정"을 언급할 때 반드시 트리거하세요.
...
mobiflow의 기획·설계 체인은 프로젝트 유형과 무관하게 고정 순서로 진행합니다. PRD에서 이 단계의 canonical form(런타임 서비스용 시스템 아키텍처 + 기술 스택)이 프로젝트에 직접 적용되지 않는다고 판단되면, 단계를 **건너뛰지 말고** 재해석해 문서를 작성합니다.
...
├── docs/            # mobiflow SSOT (spec·ui-drafts·tickets)
├── .gitignore
├── Makefile
└── README.md
```
...
```
✅ mobiflow architecture v0.1.0 작성 완료
...
다음 스킬 후보: mobiflow:spec-design-tokens
```
