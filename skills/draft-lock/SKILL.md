---
name: draft-lock
description: Working 상태인 draft 산출물(`docs/ui-drafts/`)을 commit + tag로 Lock하는 유틸리티 스킬. 사용자가 `/mobiflow:draft-lock` 슬래시 커맨드로 명시 호출할 때만 실행되며 자연어 언급으로 자동 트리거되지 않는다.
...
> ❌ `docs/ui-drafts/` 또는 `_shared/aesthetic.md`를 찾을 수 없습니다. 먼저 `/mobiflow:draft-build`로 시안을 빌드해주세요.
...
다음 단계:
  ...
  - 다음 phase: /mobiflow:compile-project-config → dev-segment-router
```
