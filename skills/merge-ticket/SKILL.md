...
| 형식                              | 동작                                                      |
| --------------------------------- | --------------------------------------------------------- |
| `/mobiflow:merge-ticket 7`          | 현재 PRD 버전의 `007` 티켓(상태 ready) merge              |
| `/mobiflow:merge-ticket 7, 11`      | 현재 PRD 버전의 `007`, `011` 순차 merge                   |
| `/mobiflow:merge-ticket all`        | 현재 PRD 버전의 **모든** `ready` 티켓을 의존 순으로 merge |
| `/mobiflow:merge-ticket v0.2.0 all` | 명시 버전의 모든 `ready` 티켓 merge                       |
| `/mobiflow:merge-ticket v0.2.0 7`   | 명시 버전의 `007` merge                                   |
...
- `pending` / `in-progress` → 에러 종료:
  ...
  > `pending`/`in-progress`는 `/mobiflow:run-dev-from-ticket {N}`으로 먼저 구현을 완료하세요.
...
- 사용자 안내:
  ...
  > 2. 본 스킬 재호출 (`/mobiflow:merge-ticket {N}`)
  >
...
다음 단계:
  - 남은 ready 있음 → /mobiflow:merge-ticket <NNN | all>
  - 남은 pending 있음 → /mobiflow:run-dev-from-ticket {번호 | v{버전}}
  - 전체 done → /mobiflow:segment-close
```
