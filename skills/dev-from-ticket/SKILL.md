...
| 형식 | 동작 |
|---|---|
| `/mobiflow:dev-from-ticket v0.2.0` | `docs/tickets/v0.2.0/` 전체 (done·ready 제외) |
| `/mobiflow:dev-from-ticket 3` | 현재 PRD 버전의 `3.md`만 |
| `/mobiflow:dev-from-ticket 7, 11` | 현재 PRD 버전의 `7.md`, `11.md` |
...
test -f docs/project.config.yaml || {
  echo "❌ docs/project.config.yaml 없음. /mobiflow:compile-project-config 실행 필요"
  exit 1
}
```
...
test -d "docs/tickets/v${TARGET_VERSION}" || { echo "❌ 티켓 없음. /mobiflow:ticket 먼저 실행"; exit 1; }
...
사용자는 worktree에서 직접 리뷰·추가 테스트·수정 commit 가능. 최종 반영은 `/mobiflow:merge-ticket {N}`.
...
처리:
  ...
  사용자 안내:
    ...
    - 복구 옵션:
      1. worktree에서 수동 완료 → ready 마킹 → /mobiflow:merge-ticket
...
다음 단계:
  - Ready 티켓 머지 → /mobiflow:merge-ticket <NNN | all>
  - 남은 pending 있음 → /mobiflow:dev-from-ticket {남은 번호}
  - 모든 done 도달 → /mobiflow:integration-verify v{TARGET_VERSION}
```
...
- **done 티켓 재오픈 금지**: 후속 티켓 append로 처리 (`/mobiflow:ticket "{설명}"`)
